...
"chain-id:" 4 1 "" 4 25 70 120 > undelegate/out.tmp \
2>&1 >/dev/tty
# Start retrieving each line from temp file 1 by one with sed and declare variables as inputs
export ADDRESS=`sed -n 1p undelegate/out.tmp`
export VALIDATOR=`sed -n 2p undelegate/out.tmp`
export AMOUNT=`sed -n 3p undelegate/out.tmp`
export CHAIN=`sed -n 4p undelegate/out.tmp`
docker exec -it node bandd tx staking unbond $VALIDATOR $AMOUNT --from $ADDRESS --chain-id $CHAIN --gas auto
echo "press enter to continue"
read
# remove temporary file created
rm -f undelegate/out.tmp
